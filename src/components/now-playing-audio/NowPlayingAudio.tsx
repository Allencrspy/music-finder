...
const NowPlayingAudio = () => {
  ...
  const handleDownload = async () => {
    if (!currentSong.previewUrl || downloadLoading) return;
    ...
    try {
      ...
      const safeName = currentSong.trackName.replace(/[^\w\s-]/g, "").trim();
      const link = document.createElement("a");

      link.href = url;
      link.download = `${safeName || "track"}.mp3`;
      ...
      <div className="player-container">
        <div className="player-left">
          ...
          <div className="player-info">
            <div className="player-title">{currentSong.trackName}</div>
            <div className="player-artist">{currentSong.artistName}</div>
          </div>
        </div>

        <div className="player-center">
          <div className="player-audio-shell">
            <span className="player-loading-slot">
              <span
                className={`player-loading-spinner ${audioLoading ? "visible" : ""}`}
                role="status"
            ...
            <audio
              ...
              onCanPlayThrough={hideAudioLoading}
              onWaiting={() => {
                showAudioLoading();
                setIsPlaying(false);
              }}
              onLoadStart={showAudioLoading}
              onError={() => {
                hideAudioLoading();
                setIsPlaying(false);
              }}
        ...
        <div className="player-right">
          <IconButton
            ...
            aria-label={downloadLoading ? "Downloading" : "Download track"}
            icon={
              <svg
...
export default NowPlayingAudio;
